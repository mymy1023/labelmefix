...
def test_shape_color_picker_previews_and_persists_only_on_accept(
    *,
    ...
    def preview_color(*, accept: bool) -> None:
        ...
        preview_checks.append(
            (
                ...
                safe_load(editable_config_file.read_text())
                == persisted_before_pick,
            )
        )
    ...
    QtCore.QTimer.singleShot(
        50,
        lambda: preview_color(accept=False),
    )
    swatch.click()
    ...
    QtCore.QTimer.singleShot(
        50,
        lambda: preview_color(accept=True),
    )
    swatch.click()
...
def test_show_labels_toggle_applies_to_canvas(
    *,
    ...
    assert (
        safe_load(editable_config_file.read_text())["shape"]["show_labels"]
        is True
    )
...
def test_flags_setting_refreshes_flag_dock_live(
    *,
    ...
    assert win._read_flag_dock_states() == {
        "occluded": False,
        "truncated": False,
    }
...
def test_setting_controls_revert_when_write_fails(
    *,
    ...
    def _refuse_write(
        ...
        **_kwargs: object,
        ...
        raise PermissionError(
            f"injected write failure: {config_file}"
        )
    ...
    assert safe_load(config_file.read_text()) == {
...
def test_keep_prev_dialog_toggle_checks_menu_action(
    *,
    ...
    assert (
        safe_load(editable_config_file.read_text())["keep_prev"]
        is True
    )
...
def test_menu_toggle_persists_and_syncs_settings_dialog(
    *,
    ...
    persisted_value = safe_load(
        editable_config_file.read_text()
    )
...
def test_sort_labels_dialog_toggle_rebuilds_label_dialog(
    *,
    ...
    old_label_dialog.destroyed.connect(
        lambda: deleted.append(True)
    )
